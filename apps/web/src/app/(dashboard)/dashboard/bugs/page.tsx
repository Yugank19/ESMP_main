...
export default function BugsPage() {
  ...
  return (
    <div className="flex flex-col space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="min-w-0">
            <h1 className="text-lg sm:text-xl font-bold text-[var(--text-primary)] uppercase tracking-tight truncate">Anomaly Tracking System</h1>
            <p className="hidden sm:block text-[10px] font-bold text-[var(--text-muted)] mt-1 uppercase tracking-widest">Central Repository for Defect Isolation and Neutralization</p>
          </div>
        </div>
      ...
      {/* Modern Filter Strip */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between bg-[var(--bg-surface-2)] p-1 sm:p-1.5 border border-[var(--border)] rounded-[3px] gap-2">
        <div className="flex items-center gap-1 overflow-x-auto no-scrollbar whitespace-nowrap">
          <div className="px-2 sm:px-3 py-2 text-[9px] sm:text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest flex items-center gap-2 border-r border-[var(--border)] mr-1 sm:mr-2 shrink-0">
            <Filter className="h-3 w-3" /> Filter
          </div>
          <button
            onClick={() => setFilterStatus('')}
            className={cn(
              "px-3 sm:px-4 py-2 rounded-[2px] text-[9px] sm:text-[10px] font-bold uppercase tracking-widest transition-all shrink-0",
