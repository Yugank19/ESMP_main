...
const TABS = [
  ...
  { id: 'privacy', label: 'Privacy', icon: Eye, desc: 'Visibility settings' },
  { id: 'team', label: 'Team Settings', icon: Globe, desc: 'Team configuration' },
];

const inputClass = "w-full px-4 py-3 bg-white border border-[var(--border)] rounded-[3px] text-sm font-semibold text-[var(--text-primary)] placeholder:text-slate-400 outline-none focus:border-[var(--color-primary)] transition-all";
const labelClass = "text-xs font-semibold text-[var(--text-secondary)] mb-2 block";
    ...
    <div className="flex flex-col space-y-8 animate-in fade-in duration-300 pb-12">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)] mt-1">Manage your account, preferences, and team configuration</p>
          </div>
        </div>
        {saved && (
            <div className="flex items-center gap-3 px-6 py-3 bg-emerald-50 border border-emerald-100 rounded-[3px] text-emerald-700 text-sm font-semibold animate-in slide-in-from-top-4 shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-2">
                ...
                {TABS.map(tab => (
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                    ...
                    <div className="flex-1 min-w-0">
                        <p className={cn("text-sm font-semibold", activeTab === tab.id ? "text-[var(--text-primary)]" : "text-slate-600 group-hover:text-slate-800")}>{tab.label}</p>
                        <p className="text-xs text-slate-400 mt-0.5 truncate">{tab.desc}</p>
                    </div>
            ...
            <div className="p-4 bg-red-50 border border-red-100 rounded-[3px] mt-6 group">
                <button onClick={handleLogout}
                    className="w-full flex items-center gap-4 py-2 text-red-500 transition-all font-semibold text-sm">
        ...
        <div className="lg:col-span-3 card p-0 overflow-hidden shadow-xl border-[var(--border)] min-h-[600px] animate-in slide-in-from-right-4 duration-500">
          <div className="bg-[var(--bg-surface-2)] border-b border-[var(--border)] px-10 py-5 flex items-center justify-between">
               <div className="flex items-center gap-3">
                    <h2 className="text-sm font-bold text-[var(--text-primary)] capitalize">{activeTab.replace('-', ' ')}</h2>
               </div>
          </div>
          ...
          <div className="p-12">
                  ...
                  <div className="flex items-center gap-8 group/root">
                    ...
                    <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                             ...
                             <span className="px-2.5 py-0.5 bg-blue-50 border border-blue-100 text-[var(--color-primary)] text-xs font-semibold rounded-[2px]">{user.roles?.[0] || 'User'}</span>
                        </div>
                        <p className="text-sm text-slate-500 flex items-center gap-2">
                             <Mail className="h-3.5 w-3.5" /> {user.email}
                        </p>
                        <div className="mt-4 flex gap-3">
                             <label className="jira-button border border-[var(--border)] h-9 px-4 bg-white text-sm font-medium cursor-pointer hover:bg-slate-50 transition-all flex items-center gap-2">
                                <input type="file" accept="image/*" className="hidden" onChange={handleAvatarUpload} />
                  ...
                  {viewPhoto && user.avatar_url && (
                    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md flex items-center justify-center z-[200] p-8 animate-in fade-in duration-300" onClick={() => setViewPhoto(false)}>
                      <div className="relative max-w-2xl w-full" onClick={e => e.stopPropagation()}>
                        ...
                        <button onClick={() => setViewPhoto(false)} className="absolute -top-12 right-0 flex items-center gap-2 text-sm font-medium text-white hover:text-[var(--color-primary)] transition-colors">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-1.5">
                        ...
                        <div className="relative">
                            ...
                            <input value={profileForm.name} onChange={e => setProfileForm(f => ({ ...f, name: e.target.value }))} className={cn(inputClass, "pl-12")} />
                        </div>
                    </div>
                    <div className="space-y-1.5">
                        ...
                        <div className="relative">
                            ...
                            <input value={profileForm.email} disabled className={cn(inputClass, "pl-12 lowercase text-slate-400 bg-slate-50")} />
                        </div>
                    </div>
                    <div className="space-y-1.5">
                        ...
                        <div className="relative">
                            ...
                            <input value={profileForm.phone} placeholder="+1 555 0000" onChange={e => setProfileForm(f => ({ ...f, phone: e.target.value }))} className={cn(inputClass, "pl-12")} />
                        </div>
                    </div>
                    <div className="space-y-1.5 md:col-span-2">
                        ...
                        <div className="relative">
                            ...
                            <textarea value={profileForm.bio} placeholder="Tell your team a bit about yourself..." onChange={e => setProfileForm(f => ({ ...f, bio: e.target.value }))} rows={4}
                                className={cn(inputClass, "pl-12 resize-none leading-relaxed")} />
                        </div>
                    </div>
                  </div>
              ...
              {activeTab === 'notifications' && prefs && (
                <div className="space-y-8">
                  <div>
                    ...
                    <p className="text-sm text-[var(--text-secondary)] mt-1">Choose how and when you receive notifications</p>
                  </div>
                  <div className="space-y-2">
                    <Toggle label="All Notifications" desc="Master switch for all notifications" icon={ShieldCheck} value={prefs.notifications_enabled}
                      onChange={(v: boolean) => setPrefs((p: any) => ({ ...p, notifications_enabled: v }))} />
                    <Toggle label="Email Notifications" desc="Receive notifications in your email inbox" icon={Mail} value={prefs.email_notifications}
                      onChange={(v: boolean) => setPrefs((p: any) => ({ ...p, email_notifications: v }))} />
                    <Toggle label="Desktop Notifications" desc="Show browser push notifications on your desktop" icon={Smartphone} value={prefs.desktop_notifications}
              ...
              {activeTab === 'appearance' && prefs && (
                <div className="space-y-10">
                  <div>
                    ...
                    <p className="text-sm text-[var(--text-secondary)] mt-1">Customize the look and feel of the interface</p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {[
                          { value: 'light', label: 'Light', icon: Zap, bg: 'bg-white', text: 'text-slate-900', border: 'border-slate-100' }, 
                          { value: 'dark', label: 'Dark', icon: Lock, bg: 'bg-slate-900', text: 'text-white', border: 'border-slate-800' }, 
                          { value: 'system', label: 'System Default', icon: Globe, bg: 'bg-gradient-to-br from-white to-slate-900', text: 'text-slate-500', border: 'border-slate-200' }
                      ].map(t => (
              ...
              {activeTab === 'security' && (
                <div className="space-y-12">
                  <div>
                    ...
                    <p className="text-sm text-[var(--text-secondary)] mt-1">Update your password and manage active sessions</p>
                  </div>
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        {passwordForm.newPass && (
                          <div className="p-4 bg-slate-50 border border-slate-100 rounded-[3px]">
                            <p className="text-xs font-semibold text-slate-500 mb-3">Password Strength</p>
                  ...
                  <div className="pt-12 border-t border-slate-50">
                    <h3 className="text-sm font-bold text-[var(--text-primary)] mb-6 flex items-center gap-3">
                        <Activity className="h-4 w-4 text-[var(--color-primary)]" /> Active Sessions
                    </h3>
                    <div className="bg-slate-50 group hover:bg-slate-100/50 border border-slate-100 rounded-[3px] p-6 flex items-center justify-between transition-all">
                      <div className="flex items-center gap-6">
                            ...
                            <div>
                                <p className="text-sm font-semibold text-[var(--text-primary)]">Current Session</p>
                                <p className="text-xs text-slate-500 mt-1">{user.email}</p>
                            </div>
                      </div>
                      <div className="flex items-center gap-6">
                          <div className="flex flex-col items-end">
                              <span className="text-xs font-semibold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-[2px] border border-emerald-100">Active</span>
                          </div>
                          <button onClick={handleLogout} className="p-3 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-[3px] border border-transparent hover:border-red-100 transition-all" title="Sign out">
              ...
              {activeTab === 'privacy' && prefs && (
                <div className="space-y-8">
                  <div>
                    ...
                    <p className="text-sm text-[var(--text-secondary)] mt-1">Control what others can see about you</p>
                  </div>
                  <div className="space-y-2">
                    <Toggle label="Show Profile to Team" desc="Let team members see your profile information" icon={Eye} value={prefs.profile_visible !== false}
                      onChange={(v: boolean) => setPrefs((p: any) => ({ ...p, profile_visible: v }))} />
                    <Toggle label="Show Online Status" desc="Let others see when you are active" icon={Activity} value={prefs.show_online_status !== false}
                      onChange={(v: boolean) => setPrefs((p: any) => ({ ...p, show_online_status: v }))} />
                    <Toggle label="Allow Direct Messages" desc="Allow team members to send you direct messages" icon={Zap} value={prefs.allow_direct_messages !== false}
                      onChange={(v: boolean) => setPrefs((p: any) => ({ ...p, allow_direct_messages: v }))} />
                    <Toggle label="Appear in Search" desc="Allow others to find you in the people search" icon={Search} value={prefs.searchable !== false}
              ...
              {activeTab === 'team' && (
                <div className="space-y-10">
                  <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-lg font-bold text-[var(--text-primary)]">Team Settings</h2>
                        <p className="text-sm text-[var(--text-secondary)] mt-1">Configure settings for your team</p>
                    </div>
                    {teams.length > 0 && (
                        <div className="relative group">
                            <Globe className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-[var(--color-primary)]" />
                            <select value={selectedTeam} onChange={e => setSelectedTeam(e.target.value)}
                                className="bg-white border border-[var(--border)] rounded-[3px] pl-10 pr-10 py-3 text-sm font-semibold text-[var(--text-primary)] shadow-sm outline-none appearance-none cursor-pointer hover:bg-slate-50 transition-all">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-1.5">
                      <label className={labelClass}>Team Visibility</label>
                    ...
                    <div className="space-y-1.5">
                      <label className={labelClass}>Default Task Priority</label>
                  ...
                  <div className="space-y-2">
                    <Toggle label="Allow Member Invites" desc="Let team members invite others to join" value={teamSettings['allow_member_invite'] === 'true'}
                      onChange={(v: boolean) => setTeamSettings(s => ({ ...s, allow_member_invite: String(v) }))} />
                    <Toggle label="Require Approval to Join" desc="New members must be approved by the team leader" value={teamSettings['require_approval'] === 'true'}
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex justify-between items-center">
          ...
          <span className={cn("text-xs font-semibold", score > 2 ? "text-emerald-500" : "text-amber-500")}>{labels[score]}</span>
      </div>
    </div>
  );
}
...
function SaveButton({ onClick, saving, label = 'Save Changes' }: { onClick: () => void; saving: boolean; label?: string }) {
  return (
    <button onClick={onClick} disabled={saving}
      className="jira-button jira-button-primary h-11 px-8 gap-2 font-semibold text-sm shadow-lg shadow-blue-100 disabled:opacity-50 group">
